<?php
/**
 * MerchantAttributes — Grouped attribute lists for each merchant channel.
 *
 * V8 equivalent of V5's MerchantAttributesFactory.
 * Returns the attribute option groups used by the admin UI for channel
 * mapping dropdowns (e.g., "Basic Information", "Availability & Price").
 *
 * Each merchant key maps to a numbered array of option groups, where each
 * group has an 'optionGroup' label and an 'options' associative array of
 * attribute_key => display_label.
 *
 * Usage:
 *   MerchantAttributes::get( 'google' )   → array of option groups for Google
 *   MerchantAttributes::get()             → all merchants
 *
 * @package    CTXFeed
...
class MerchantAttributes {
	...
	/**
	 * Get merchant attributes for a specific template or all merchants.
	 *
	 * Handles merchant aliases (pinterest → google, connexity → become, etc.)
	 * and applies the `ctxfeed_merchant_attributes` filter.
	 *
	 * @since 8.0.0
	 *
	 * @param string|null $template Merchant/template key (e.g., 'google', 'facebook').
	 *                              Pass null to get all merchants.
	 * @return array|false Attribute groups array, or false if template not found.
	 */
	public static function get( $template = null ) {
		...
		// Merchant aliases — some merchants share the same attribute structure.
		// Pinterest is NOT aliased: it has its own dedicated section below
		// (owner decision July 2026 — the alias used to shadow it).
		if ( in_array( $template, array( 'adroll', 'smartly.io', 'perplexity' ), true ) ) {
			$template = 'google';
		} elseif ( in_array( $template, array( 'connexity', 'shopzilla' ), true ) ) {
			$template = 'become';
		} elseif ( 'fruugo.au' === $template ) {
			$template = 'fruugo';
		} elseif ( 'shopalike.fr' === $template ) {
			$template = 'kijiji.ca';
		}

		if ( isset( $data[ $template ] ) ) {
			/**
			 * Filter merchant attributes for a specific provider.
			 *
			 * @since 8.0.0
			 *
			 * @param array  $attributes Option groups for this merchant.
			 * @param string $template   Merchant/template key.
			 */
			return apply_filters( 'ctxfeed_merchant_attributes', $data[ $template ], $template );
		}
	...
	/**
	 * Merchant attribute data registry.
	 *
	 * Returns the full map of merchant_key => option_groups.
	 *
	 * TODO: Paste the full data array from V5 MerchantAttributesFactory::data().
	 *       Copy everything inside the return array(...) from V5.
	 *
	 * @since 8.0.0
	 *
	 * @return array
	...
	private static function data(): array {
		return array(
			'google'                 => array(
				...
				2  => array(
					...
					'options'     => array(
						...
						'maximum_retail_price'      => 'Maximum Retail Price (India)[maximum_retail_price]',
					),
				),
				3  => array(
				...
				5  => array(
					...
					'options'     => array(
						// g:tax is offered for Google again (BUG-0053, owner-confirmed):
						// US merchants can rely on Google's auto-calculated tax, but
						// merchants in other countries need to declare explicit per-country
						// tax rates. g:tax is a nested group (country/region/rate/tax_ship)
						// handled by the existing TaxResolver plumbing. tax_category stays
						// alongside it as a distinct, still-valid attribute.
						'tax'                     => 'Tax[tax]',
				...
				10 => array(
					...
					'options'     => array(
						...
						'popularity_rank'                => 'Popularity Rank[popularity_rank]',
						'disclosure_date'                => 'Disclosure Date[disclosure_date]',
						'loyalty_program_label'          => 'Loyalty Program Label[program_label]',
						'loyalty_tier_label'             => 'Loyalty Tier Label[tier_label]',
						'loyalty_program_price'          => 'Loyalty Member Price[price]',
						'loyalty_cashback'               => 'Loyalty Cashback[cashback_for_future_use]',
						'loyalty_program_points'         => 'Loyalty Program Points[loyalty_points]',
						'loyalty_member_price_date'      => 'Loyalty Member Price Date[member_price_effective_date]',
						'loyalty_program_shipping_label' => 'Loyalty Shipping Label[shipping_label]',
						'qa_question'                    => 'Question[question]',
						'qa_answer'                      => 'Answer[answer]',
						'related_relationship_type'      => 'Related Relationship Type[relationship_type]',
						'related_identifier_type'        => 'Related Identifier Type[identifier_type]',
						'related_identifier'             => 'Related Identifier[identifier]',
						'variant_option_name'            => 'Variant Option Name[name]',
						'variant_option_value'           => 'Variant Option Value[value]',
				...
				12 => array(
					...
					'options'     => array(
						...
						// Google certification is a nested group: authority + name + code.
						'certification_authority'     => 'Certification Authority[certification_authority]',
						'certification_name'          => 'Certification Name[certification_name]',
						'certification_code'          => 'Certification Code[certification_code]',
			...
			'facebook'               => array(
				...
				2 => array(
					...
					'options'     => array(
						'availability'               => 'Stock Status[availability]',
						'availability_date'          => 'Availability Date[availability_date]',
						'availability_circle_origin' => 'Availability Circle Origin[availability_circle_origin]',
						'price'                      => 'Regular Price[price]',
						'sale_price'                 => 'Sale Price[sale_price]',
						'sale_price_effective_date'  => 'Sale Price Effective Date[sale_price_effective_date]',
					),
				...
				7 => array(
					...
					'options'     => array(
						...
						'video'                        => 'Video [video]',
						'custom_number_0'              => 'Custom Number 0 [custom_number_0]',
						'custom_number_1'              => 'Custom Number 1 [custom_number_1]',
						'custom_number_2'              => 'Custom Number 2 [custom_number_2]',
						'custom_number_3'              => 'Custom Number 3 [custom_number_3]',
						'custom_number_4'              => 'Custom Number 4 [custom_number_4]',
						'vendor_id'                    => 'Vendor ID [vendor_id]',
						'base_price'                   => 'Base Price [base_price]',
						'wa_compliance_category'       => 'WA Compliance Category [wa_compliance_category]',
						'disabled_capabilities'        => 'Disabled Capabilities [disabled_capabilities]',
						'applink_ios_url'              => 'App Link > iOS URL [applink.ios_url]',
						'applink_ios_app_store_id'     => 'App Link > iOS App Store ID [applink.ios_app_store_id]',
						'applink_ios_app_name'         => 'App Link > iOS App Name [applink.ios_app_name]',
						'applink_android_url'          => 'App Link > Android URL [applink.android_url]',
						'applink_android_package'      => 'App Link > Android Package [applink.android_package]',
						'applink_android_app_name'     => 'App Link > Android App Name [applink.android_app_name]',
			...
			'x'                      => array(
				...
				3 => array(
					'optionGroup' => 'Identifiers & Variants',
					'options'     => array(
						'brand' => 'Brand[brand]',
						'gtin'  => 'GTIN[gtin]',
						'mpn'   => 'MPN[mpn]',
			...
			'reddit'                 => array(
				...
				3 => array(
					'optionGroup' => 'Product Identifiers & Details',
					'options'     => array(
						'brand'          => 'Brand[brand]',
						'gtin'           => 'GTIN[gtin]',
						'mpn'            => 'MPN[mpn]',
			...
			'pinterest'              => array(
				...
				19 => array(
					'optionGroup' => 'Pinterest 2026 Attributes',
					'options'     => array(
						'ad_link'          => 'Ad Link[ad_link]',
						'ai_disclosures'   => 'AI Disclosures[ai_disclosures]',
						'alt_text'         => 'Image Alt Text[alt_text]',
						'variant_names'    => 'Variant Names[variant_names]',
						'variant_values'   => 'Variant Values[variant_values]',
						'description_html' => 'Description HTML[description_html]',
						'video_link'       => 'Video Link[video_link]',
						'custom_number_0'  => 'Custom Number 0 [custom_number_0]',
						'custom_number_1'  => 'Custom Number 1 [custom_number_1]',
						'custom_number_2'  => 'Custom Number 2 [custom_number_2]',
			...
			'skroutz'                => array(
				...
				4 => array(
					'optionGroup' => 'VAT & Additional Fields',
					'options'     => array(
						'vat'                 => 'VAT Rate[vat]',
						'shipping'            => 'Shipping Cost[shipping]',
						'wholesale_price'     => 'Wholesale Price[wholesale_price]',
						'season'              => 'Season[season]',
						'size_fit'            => 'Size Fit[size_fit]',
						'outlet'              => 'Outlet[outlet]',
						'author'              => 'Author[author]',
						'country_of_origin'   => 'Country of Origin[country_of_origin]',
						'expiration_date'     => 'Expiration Date[expiration_date]',
						'specification_name'  => 'Specification Name[specification_name]',
						'specification_value' => 'Specification Value[specification_value]',
					),
				),
				3 => array(
					'optionGroup' => 'Variations',
					'options'     => array(
						'variation_id'             => 'Variation ID [variationid]',
						'variation_size'           => 'Variation Size [size]',
						'variation_quantity'       => 'Variation Quantity [quantity]',
						'variation_availability'   => 'Variation Availability [availability]',
						'variation_ean'            => 'Variation EAN [ean]',
						'variation_mpn'            => 'Variation MPN [mpn]',
						'variation_price_with_vat' => 'Variation Price With VAT [price_with_vat]',
						'variation_link'           => 'Variation Link [link]',
						'variation_outlet'         => 'Variation Outlet [outlet]',
					),
				),
			),
			'daisycon'               => array(
			...
			'chatgpt'                => array(
				...
				9  => array(
					...
					'options'     => array(
						...
						'store_country'         => 'Store country (2-letter ISO code)',
						'target_countries'      => 'Countries where the product is available',
					),
				),
				10 => array(
					'optionGroup' => 'Returns',
